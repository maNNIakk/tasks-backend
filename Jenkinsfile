pipeline {
    ...
    stages {
        ...
        stage('Health Check'){
            steps{
                sleep(5)
                dir('functional-test'){
                    bat 'mvn verify -Dtest=!TasksTest'
                }
            }
        }
    }
    ...
    post {
        ...
        unsuccessful {
            emailext attachLog: true, body: 'See the attached log below', subject: 'Build $BUILD_NUMBER has failed', to: 'renato.yancovit3+jenkins@gmail.com'
        }
        fixed {
            emailext attachLog: true, body: '', subject: 'Build fixed!!', to: 'renato.yancovit3+jenkins@gmail.com'
        }
    }
}
